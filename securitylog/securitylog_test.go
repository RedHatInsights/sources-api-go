...
func testXRHIDWithUser(orgID, userID string) *identity.XRHID {
	return &identity.XRHID{
		Identity: identity.Identity{
			AccountNumber:         "",
			EmployeeAccountNumber: "",
			OrgID:                 orgID,
			Internal:              identity.Internal{OrgID: "", AuthTime: 0, CrossAccess: false},
			User: &identity.User{
				Username:  "",
				Email:     "",
				FirstName: "",
...
func testXRHIDWithServiceAccount(orgID, clientID string) *identity.XRHID {
	return &identity.XRHID{
		Identity: identity.Identity{
			AccountNumber:         "",
			EmployeeAccountNumber: "",
...
func testXRHIDWithSystem(orgID, commonName string) *identity.XRHID {
	return &identity.XRHID{
		Identity: identity.Identity{
			AccountNumber:         "",
			EmployeeAccountNumber: "",
...
func TestGetPrincipalUser(t *testing.T) {
	c := createTestContext(http.MethodGet, testXRHIDWithUser("org-123", "user-456"))
...
func TestGetPrincipalServiceAccount(t *testing.T) {
	c := createTestContext(http.MethodGet, testXRHIDWithServiceAccount("org-789", "sa-client-1"))
...
func TestGetPrincipalSystem(t *testing.T) {
	c := createTestContext(http.MethodGet, testXRHIDWithSystem("org-sys", "satellite-cn"))
...
func TestLogCrud(t *testing.T) {
	buf := setupTestLogger()
	c := createTestContext(http.MethodPost, testXRHIDWithUser("12345", "user-1"))
...
func TestLogAuthzFailure(t *testing.T) {
	buf := setupTestLogger()
	c := createTestContext(http.MethodPost, testXRHIDWithUser("org-denied", "user-denied"))
