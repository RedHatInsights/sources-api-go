...
// Uses jwx library's built-in caching with 15-minute refresh interval.
...
// LRU cache with 100 issuer capacity, 1-hour TTL, stale-while-revalidate pattern.
// Thread-safe for concurrent access.
var discoveryCache, _ = lru.New[string, CachedJWKSURL](100)
...
// 2. JWKS content cache (15-minute refresh interval with automatic background updates)
//
// Flow:
...
func getJWKSImpl(ctx context.Context, issuer string) (jwk.Set, error) {
	...
	// Register JWKS URL with 15-minute refresh interval only if not already registered
