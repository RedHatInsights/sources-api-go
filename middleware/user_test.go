package middleware

import (
	"net/http"
	"testing"

	"github.com/RedHatInsights/sources-api-go/dao"
	"github.com/RedHatInsights/sources-api-go/internal/testutils"
	"github.com/RedHatInsights/sources-api-go/internal/testutils/database"
	"github.com/RedHatInsights/sources-api-go/internal/testutils/request"
	h "github.com/RedHatInsights/sources-api-go/middleware/headers"
	m "github.com/RedHatInsights/sources-api-go/model"
	"github.com/labstack/echo/v4"
)

var catchUserOrElse204 = UserCatcher(func(c echo.Context) error {
	return c.NoContent(http.StatusNoContent)
})

func TestUserCreationFromXRHID(t *testing.T) {
	testutils.SkipIfNotRunningIntegrationTests(t)

	tenantID := int64(1)
	testUserID := "55555"
	identity := testutils.IdentityHeaderForUser(testUserID)

	database.ConnectAndMigrateDB("middleware")
	database.CreateFixtures("middleware")

	c, rec := request.CreateTestContext(
		http.MethodGet,
		"/",
		nil,
		map[string]interface{}{},
	)

	c.Set(h.TenantID, tenantID)
	c.Set(h.ParsedIdentity, identity)

	err := catchUserOrElse204(c)
	if err != nil {
		t.Errorf("caught an error when there should not have been one: %v", err)
	}

	if rec.Code != 204 {
		t.Errorf("%v was returned instead of %v", rec.Code, 204)
	}

	var user m.User

	result := dao.DB.Find(&user, "user_id = ? AND tenant_id = ?", testUserID, tenantID)
	if result.Error != nil {
		t.Error(err)
	}

	if result.RowsAffected == 0 {
		t.Errorf("unable to find user %v", testUserID)
	}

	if user.Id == 0 || c.Get(h.UserID) != user.Id {
		t.Errorf("unable to find user id %v in context", user.Id)
	}

	database.DropSchema("middleware")
}

func TestUserCreationFromPSK(t *testing.T) {
	testutils.SkipIfNotRunningIntegrationTests(t)

	tenantID := int64(1)
	testUserID := "55555"

	database.ConnectAndMigrateDB("middleware")
	database.CreateFixtures("middleware")

	c, rec := request.CreateTestContext(
		http.MethodGet,
		"/",
		nil,
		map[string]interface{}{},
	)

	c.Set(h.TenantID, tenantID)
	c.Set(h.PSKUserID, testUserID)

	err := catchUserOrElse204(c)
	if err != nil {
		t.Errorf("caught an error when there should not have been one: %v", err)
	}

	if rec.Code != 204 {
		t.Errorf("%v was returned instead of %v", rec.Code, 204)
	}

	var user m.User

	result := dao.DB.Find(&user, "user_id = ? AND tenant_id = ?", testUserID, tenantID)
	if result.Error != nil {
		t.Error(err)
	}

	if user.Id == 0 || c.Get(h.UserID) != user.Id {
		t.Errorf("unable to find user id %v in context", user.Id)
	}

	if result.RowsAffected == 0 {
		t.Errorf("unable to find user %v", testUserID)
	}

	database.DropSchema("middleware")
}

func TestUserCreationFromJWT(t *testing.T) {
	testutils.SkipIfNotRunningIntegrationTests(t)

	tenantID := int64(1)
	testJWTSubject := "jwt-user-12345"

	database.ConnectAndMigrateDB("middleware")
	database.CreateFixtures("middleware")

	c, rec := request.CreateTestContext(
		http.MethodGet,
		"/",
		nil,
		map[string]interface{}{},
	)

	c.Set(h.TenantID, tenantID)
	c.Set(h.JWTSubject, testJWTSubject)

	err := catchUserOrElse204(c)
	if err != nil {
		t.Errorf("caught an error when there should not have been one: %v", err)
	}

	if rec.Code != 204 {
		t.Errorf("%v was returned instead of %v", rec.Code, 204)
	}

	var user m.User

	result := dao.DB.Find(&user, "user_id = ? AND tenant_id = ?", testJWTSubject, tenantID)
	if result.Error != nil {
		t.Error(err)
	}

	if user.Id == 0 || c.Get(h.UserID) != user.Id {
		t.Errorf("unable to find user id %v in context", user.Id)
	}

	if result.RowsAffected == 0 {
		t.Errorf("unable to find user %v", testJWTSubject)
	}

	database.DropSchema("middleware")
}

func TestUserPriorityOrder(t *testing.T) {
	testutils.SkipIfNotRunningIntegrationTests(t)

	tenantID := int64(1)
	pskUserID := "psk-user"
	testJWTSubject := "jwt-user"
	identityUserID := "identity-user"

	database.ConnectAndMigrateDB("middleware")
	database.CreateFixtures("middleware")

	c, rec := request.CreateTestContext(
		http.MethodGet,
		"/",
		nil,
		map[string]interface{}{},
	)

	c.Set(h.TenantID, tenantID)

	// Set all three auth methods - PSK should take priority
	c.Set(h.PSKUserID, pskUserID)
	c.Set(h.JWTSubject, testJWTSubject)

	identity := testutils.IdentityHeaderForUser(identityUserID)
	c.Set(h.ParsedIdentity, identity)

	err := catchUserOrElse204(c)
	if err != nil {
		t.Errorf("caught an error when there should not have been one: %v", err)
	}

	if rec.Code != 204 {
		t.Errorf("%v was returned instead of %v", rec.Code, 204)
	}

	// Verify PSK user was created (priority)
	var user m.User

	result := dao.DB.Find(&user, "user_id = ? AND tenant_id = ?", pskUserID, tenantID)
	if result.Error != nil {
		t.Error(err)
	}

	if result.RowsAffected == 0 {
		t.Errorf("PSK user should have been created but was not found: %v", pskUserID)
	}

	if user.Id == 0 || c.Get(h.UserID) != user.Id {
		t.Errorf("PSK user should be in context, got %v instead of %v", c.Get(h.UserID), user.Id)
	}

	database.DropSchema("middleware")
}
